...
function getPergunta($pergunta_id) {
  global $conn;
  $stmt = $conn->prepare("
        SELECT pergunta.pergunta_id, pergunta.titulo, pergunta.conteudo,pergunta.created_date,utilizador.avatar, utilizador.username, pergunta.criar_id, avg(votoutilizadorpergunta.valor) as average
        FROM pergunta
        JOIN utilizador ON (pergunta.criar_id = utilizador.user_id)
...
function getPerguntaRespostas($pergunta_id) {
    global $conn;
    $stmt = $conn->prepare("
        SELECT resposta.resposta_id, resposta.conteudo,resposta.created_date , utilizador.username,utilizador.avatar, avg(votoutilizadorresposta.valor) as average
        FROM resposta
